export abstract class ENGrid {
  ...
  static getGiftProcess() {
    if ("pageJson" in window) return (window as any).pageJson.giftProcess;
    return null;
  }

  // Return the page count
  static getPageCount() {
    if ("pageJson" in window) return (window as any).pageJson.pageCount;
    return null;
  }

  // Return the current page number
  static getPageNumber() {
    if ("pageJson" in window) return (window as any).pageJson.pageNumber;
    return null;
  }
  ...
  static getClientID() {
    if ("pageJson" in window) return (window as any).pageJson.clientId;
    return 0;
  }
  ...
  static getPageType() {
    if ("pageJson" in window && "pageType" in (window as any).pageJson) {
      switch ((window as any).pageJson.pageType) {
        case "donation":
        case "premiumgift":
          return "DONATION";
          break;
